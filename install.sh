...
do_copy()
{
    tar cf - -C ${DISTRO} macula | tar xvf - -C ${TARGET}
    if [ -n "$STATE_NUM" ]; then
        echo $DEVICE $STATE_NUM > $TARGET/macula/system/growpart
    fi

    if [ -n "$MACULA_INSTALL_CONFIG_URL" ]; then
        get_url "$MACULA_INSTALL_CONFIG_URL" ${TARGET}/macula/system/config.yaml
        chmod 600 ${TARGET}/macula/system/config.yaml
    fi

    if [ "$MACULA_INSTALL_TAKE_OVER" = "true" ]; then
        touch ${TARGET}/macula/system/takeover

        if [ "$MACULA_INSTALL_POWER_OFF" = true ] || grep -q 'macula.install.power_off=true' /proc/cmdline; then
            touch ${TARGET}/macula/system/poweroff
        fi
    fi
}
...
install_grub()
{
    if [ "$MACULA_INSTALL_DEBUG" ]; then
        GRUB_DEBUG="macula.debug"
    fi
...
menuentry "MaculaOS Current" {
  ...
  initrd /macula/system/kernel/current/initrd
}
...
menuentry "MaculaOS Previous" {
  ...
  set root=(\$root)
  linux (loop0)/vmlinuz printk.devkmsg=on console=tty1 $GRUB_DEBUG
  initrd /macula/system/kernel/previous/initrd
}
...
EOF
    if [ -z "${MACULA_INSTALL_TTY}" ]; then
        TTY=$(tty | sed 's!/dev/!!')
    else
        TTY=$MACULA_INSTALL_TTY
    fi
    ...
    if [ "$MACULA_INSTALL_FORCE_EFI" = "true" ]; then
...
get_iso()
{
    ...
    if [ -z "${ISO_DEVICE}" ] && [ -n "$MACULA_INSTALL_ISO_URL" ]; then
        TEMP_FILE=$(mktemp maculaos.XXXXXXXX.iso)
        get_url ${MACULA_INSTALL_ISO_URL} ${TEMP_FILE}
        ISO_DEVICE=$(losetup --show -f $TEMP_FILE)
        rm -f $TEMP_FILE
    fi

    if [ -z "${ISO_DEVICE}" ]; then
        echo "#### There is no MaculaOS ISO device"
        return 1
    fi
}
...
setup_style()
{
    if [ "$MACULA_INSTALL_FORCE_EFI" = "true" ] || [ -e /sys/firmware/efi ]; then
        PARTTABLE=gpt
        BOOTFLAG=esp
...
create_opt()
{
    mkdir -p "${TARGET}/macula/data/opt"
}

while [ "$#" -gt 0 ]; do
    case $1 in
        --no-format)
            MACULA_INSTALL_NO_FORMAT=true
            ;;
        --force-efi)
            MACULA_INSTALL_FORCE_EFI=true
            ;;
        --poweroff)
            MACULA_INSTALL_POWER_OFF=true
            ;;
        --takeover)
            MACULA_INSTALL_TAKE_OVER=true
            ;;
        --debug)
            set -x
            MACULA_INSTALL_DEBUG=true
            ;;
        --config)
            shift 1
            MACULA_INSTALL_CONFIG_URL=$1
            ;;
        --tty)
            shift 1
            MACULA_INSTALL_TTY=$1
            ;;
        -h)
            usage
            ;;
        ...
        *)
            ...
            INTERACTIVE=true
            MACULA_INSTALL_DEVICE=$1
            MACULA_INSTALL_ISO_URL=$2
            break
            ;;
    esac
    shift 1
...
if [ -e /etc/os-release ]; then
    ...
    if [ -z "$MACULA_INSTALL_ISO_URL" ]; then
        MACULA_INSTALL_ISO_URL=${ISO_URL}
    fi
fi

if [ -z "$MACULA_INSTALL_DEVICE" ]; then
    usage
fi
